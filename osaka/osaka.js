...
var cam = require('nsc')( canvas, {
  ...
  rotation: mat4.fromRotation( mat4.create(), Math.PI * 0.25, [ 0, 1, 0 ] )
} );
var cga = require('cga');
var lgp = require('lgp');
var mda = require('mda');
...
function setupGeometry() {
  ...
  // lgp.fileReader( './../osaka/crystal.obj', function parseObj( text ) {
    ...
    ScaleOperator( othermesh, 0.9 );
    InvertOperator( othermesh );
    var len = omesh.getFaces().length;
    for( var i = 0; i < len; i++ ) {
      for( var j = 0; j < 1; j++ ) {
        ExtrudeOperator( omesh, i, 0.0, 1.0 );
        ExtrudeOperator( omesh, i, 1.0, -0.5 );
        ExtrudeOperator( omesh, i, 0.0, 1.0 );
        ExtrudeOperator( omesh, i, -4.0, 3.0 );
      }
    }
