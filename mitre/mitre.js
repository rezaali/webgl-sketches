...
var cam = require('nsc')( canvas, {
  ...
  // rotation: mat4.fromQuat( mat4.create(), quat.rotationTo( quat.create(), [ 0, 0, 1 ], [ 0, -0.3, 0 ] ) )
} );
...
function setupGeometry() {
  lgp.fileReader( path, function parseObj( text ) {
    ...
    WireframeOperator( omesh, 3.0, 3.0 );
...
window.addEventListener( 'keydown', function( event ) {
  if( keyPressed( 'S' ) ) {
    lgp.imageWriter( 'mitre.png', canvas.toDataURL('image/png') );
    return;
  }
  if( keyPressed( 'E' ) ) {
