...
var fileReader = lgp.fileReader( path, function parseObj( text ) {
  ...
  var newWirePositions = [];
  var wirePositions = results.positions;
  var wireCells = results.cells;
  var wlen = wirePositions.length;
  for( var i = 0; i < wlen; i++ ) {
    newWirePositions.push( wirePositions[ i ][ 0 ] );
    newWirePositions.push( wirePositions[ i ][ 1 ] );
    newWirePositions.push( wirePositions[ i ][ 2 ] );
  }
  ...
  geoWire.attr( 'aPosition', newWirePositions );
  geoWire.faces( lines, { size: 2 } )

  geoPoints = glGeometry( gl );
  geoPoints.attr( 'aPosition', newWirePositions );
} );
...
// Setup Shaders
var vertexShader = glslify( './shaders/LoopSmoothShader.vert' );
var fragmentShader = glslify( './shaders/LoopSmoothShader.frag' );
var shader = glShader( gl, vertexShader, fragmentShader );

var vertexWireframeShader = glslify( './shaders/LoopSmoothDebugShader.vert' );
var fragmentWireframeShader = glslify( './shaders/LoopSmoothDebugShader.frag' );
var shaderDebug = glShader( gl, vertexWireframeShader, fragmentWireframeShader );

var color = [ 1.0, 1.0, 1.0, 1.0 ];
var colorPoints = [ 1.0, 1.0, 1.0, 0.75 ];
...
window.addEventListener( 'keydown', function( event ) {
  if( keyPressed( 'S' ) ) {
    lgp.imageWriter( 'loopsmooth.png', canvas.toDataURL('image/png') );
  }
}, false );
